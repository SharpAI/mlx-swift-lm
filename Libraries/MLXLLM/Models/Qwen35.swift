...
public class Qwen35TextModel: Module, LLMModel, KVCacheDimensionProvider {
    ...
    public func sanitize(weights: [String: MLXArray]) -> [String: MLXArray] {
        ...
        //
        // hasUnsanitizedConv1d is a reliable signal on its own (the conv1d layout is
        // unambiguous) and always triggers the shift. hasMTPWeights is not proof of a
        // raw checkpoint — a converted checkpoint can keep MTP tensors (the framework
        // uses them for speculative decoding) — so it only triggers the shift when the
        // norm-statistics check above also agrees the checkpoint looks unshifted.
