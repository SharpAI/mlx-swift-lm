...
public class GLMMoeDSAModel: Module, LLMModel, KVCacheDimensionProvider {
    public let vocabularySize: Int
    public let kvHeads: [Int]

    public let model: GLMMoeDSAModelInner
    let configuration: GLMMoeDSAConfiguration

    @ModuleInfo(key: "lm_head") var lmHead: Linear

    public init(_ args: GLMMoeDSAConfiguration) {
        self.configuration = args
        self.vocabularySize = args.vocabularySize
        self.kvHeads = (0 ..< args.hiddenLayers).map { _ in args.kvHeads }
        self.model = GLMMoeDSAModelInner(args)
...
public class GLMMoeDSAModelInner: Module {
    @ModuleInfo(key: "embed_tokens") var embedTokens: Embedding
    let norm: RMSNorm
    
    init(_ config: GLMMoeDSAConfiguration) {
