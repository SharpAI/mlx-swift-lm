...
final class Gemma4TextAttention: Module {
    ...
    init(config: Gemma4TextConfiguration, layerIdx: Int, kvSharedOnly: Bool = false) {
        ...
        // K/V and own no k_proj/v_proj/k_norm — the checkpoint ships none, so the
        // module tree must not declare them (else loadWeights fails keyNotFound on the
        // shared layers, e.g. E2B layer 15 / E4B layer 24 — SwiftLM #120). `kvSharedOnly`
        // is the drafter's always-shared variant; `isKVSharedLayer` covers the target's
        // own shared tail. Mirrors the text backbone (MLXLLM/Models/Gemma4Text.swift).
        // v_norm is parameter-free (Gemma4RMSNormNoScale) and always built regardless —
        // every layer needs it, shared or not, matching the text backbone.
...
public final class Gemma4: Module, VLMModel, KVCacheDimensionProvider, StreamableMoE {
    ...
    private func getInputEmbeddings(
        ...
        if let audio, let audioTower, let embedAudio, let audioTokenId = config.audioTokenId {
            let actualAudioMask = audio.mask ?? MLXArray.ones(audio.features.shape[0..<2], dtype: .bool)
            let (audioOutputs, _) = audioTower(audio.features, mask: actualAudioMask)
            eval(audioOutputs)
...
public struct Gemma4MessageGenerator: MessageGenerator {
    ...
    public func generate(message: Chat.Message) -> MLXLMCommon.Message {
        ...
        if message.role != .user || (message.images.isEmpty && message.videos.isEmpty && message.audios.isEmpty) {
...
public struct Gemma4Processor: UserInputProcessor {
    ...
    public func prepare(input: UserInput) async throws -> LMInput {
        ...
        return LMInput(
            ...
            video: processedVideo, audio: processedAudio)
    }
}
